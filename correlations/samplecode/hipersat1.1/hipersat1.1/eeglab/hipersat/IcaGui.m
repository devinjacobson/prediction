...
% ICAGUI M-file for IcaGui.fig
%
%      ICAGUI, by itself, creates a new ICAGUI or raises the existing
%      singleton.
%
%      H = ICAGUI returns the handle to a new ICAGUI or the handle to
%      the existing singleton*.
%
%      ICAGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ICAGUI.M with the given input arguments.
%
%      ICAGUI('Property','Value',...) creates a new ICAGUI or raises
%      the existing singleton.  Starting from the left, property value pairs are
%      applied to the GUI before IcaGui_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to IcaGui_OpeningFcn via varargin.
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @IcaGui_OpeningFcn, ...
                   'gui_OutputFcn',  @IcaGui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before IcaGui is made visible.

function IcaGui_OpeningFcn(hObject, eventdata, handles, varargin);

% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to IcaGui (see VARARGIN)
...
function varargout = IcaGui_OutputFcn(hObject, eventdata, handles);
...
switch get(hObject, 'Value')
    ...
    case 1
        ...
        [sigClean.scfName sigClean.scfPath] = uigetfile('*.scf', 'Select HiPerSAT (.scf) Data File');
        
        figure(handles.IcaGui);
        
        if isequal(sigClean.scfName, 0)
            
            set(handles.NumChanEditBox, 'String', '');
            set(handles.NumSmplEditBox, 'String', '');
            
            set( handles.ScfTypePopUp,   ...
                 ...
                 'Enable', 'On');
                 
            set( handles.PathEditBox,    ...
                 ...
                 'String', '>>> Path To .scf Data File'    );
            ...
            [path file ext] = fileparts(sigClean.scfName);
            ...
            if strcmp(ext, '.scf')
                ...
                set( handles.PathEditBox,    ...
                     ...
                     'String', fullfile(sigClean.scfPath, sigClean.scfName) );
                
                set( handles.IcaGui, ...
                     ...
                     'Name', [ 'HiPerSAT: ICA Form Editor << ' sigClean.scfName ' >>' ] );
        ...
        if errorFlag
            ...
            errordlg('--- Invalid Input: Please Select A Valid HiPerSAT (.scf) File ---', 'User Input Error');
        
        end
        
    case 2
        
        [sigClean.scfPath, sigClean.scfName, sigClean.numChan, sigClean.numSmpl] = FileOpenGUI;
        ...
        if isequal(sigClean.scfName, 0)
            ...
            set(handles.NumChanEditBox, 'String', int2str(sigClean.numChan));
            set(handles.NumSmplEditBox, 'String', int2str(sigClean.numSmpl));
            
            set( handles.ScfTypePopUp,      ...
                 ...
                 'Enable', 'Inactive',      ...
                 'Value',  sigClean.scfFormat );
...
% --- Executes on selection change in ScfTypePopUp.

function ScfTypePopUp_Callback(hObject, eventdata, handles);

% hObject    handle to ScfTypePopUp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global sigClean;

sigClean.scfFormat = get(hObject, 'Value');



% --- Executes during object creation, after setting all properties.

function ScfTypePopUp_CreateFcn(hObject, eventdata, handles);
...
if isequal(exist(get(handles.PathEditBox, 'String'), 'file'), 0)
        
    errorFlag = 1;
        
else
        
    [path file ext] = fileparts(get(handles.PathEditBox, 'String'));
        
    if strcmp(ext, '.scf')
            
        errorFlag = 0;

        sigClean.scfPath = path;
        sigClean.scfName = [file ext];
...
function NumChanEditBox_Callback(hObject, eventdata, handles);

% hObject    handle to NumChanEditBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global sigClean;

sigClean.numChan = str2num(get(hObject, 'String'));

if isempty(sigClean.numChan) || imag(sigClean.numChan) || (sigClean.numChan <= 0) || mod(sigClean.numChan, 1)
    
    errordlg('--- Invalid Input: Number Of Channels Field ---', 'User Input Error');
    set(hObject, 'String', '');
    sigClean.numChan = 0;
...
function NumChanEditBox_CreateFcn(hObject, eventdata, handles);
...
function NumSmplEditBox_Callback(hObject, eventdata, handles);

% hObject    handle to NumSmplEditBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global sigClean;

sigClean.numSmpl = str2num(get(hObject, 'String'));

if isempty(sigClean.numSmpl) || imag(sigClean.numSmpl) || (sigClean.numSmpl <= 0) || mod(sigClean.numSmpl, 1)
    
    errordlg('--- Invalid Input: Number Of Samples Field ---', 'User Input Error');
    set(hObject, 'String', '');
    sigClean.numSmpl = 0;
...
function NumSmplEditBox_CreateFcn(hObject, eventdata, handles);
...
if (sigClean.wgtMtrx == 3)
    ...
    if ~sigClean.load
        ...
        [sigClean.wgtFileName, sigClean.wgtFilePath] = uigetfile('*.uwgt', 'Select Weight Matrix Seed');
...
if (sigClean.wgtMtrx2 == 3)
    ...
    if ~sigClean.load
        ...
        [sigClean.wgtFile2Name, sigClean.wgtFile2Path] = uigetfile('*.uwgt', 'Select Weight Matrix Seed');
...
if isequal(sigClean.scfName, 0)
    ...
    errordlg('--- Invalid State: Please Select A Valid HiPerSAT (.scf) File ---', 'User Request Error');
    
    return;
    
end

if sigClean.numChan == 0 || sigClean.numSmpl == 0
    
    errordlg('--- Invalid Input: Please Select Valid Channel / Sample Data Values ---', 'User Input Error');
...
[formName, formPath] = uiputfile( ['*' sigClean.formExt], 'Save HiPerSAT Form', [sigClean.scfName sigClean.formExt] );
...
if isequal(formName, 0)
    ...
    if strcmp(ext, sigClean.formExt)
        ...
        NicICAForm(formPath, formName, sigClean);
...
set( handles.IcaGui, 'Name', [ 'HiPerSAT: ICA Form Editor << ' formName ' >>' ] );

sigClean.frmName = formName;
...
fprintf( '\nDetermining HiPerSAT configuration ...\n' );
...
eval(['! ' hipersatExec ' ' fullfile(sigClean.scfPath, sigClean.frmName) ' ' sigClean.scfPath]);
...
formMap = struct( 'Source_data_location',               { 'DefaultField'   ,    @ScfPath         },  ...
                  'File_type',                          { 'DefaultField'   ,    @ScfType         },  ...
                  'Channels',                           { 'numChan'        ,    @PassToDouble    },  ...
                  'Observations',                       { 'numSmpl'        ,    @PassToDouble    },  ...
                  'Process',                            { 'DefaultField'   ,    @IcaProtocol     },  ...                  
                  ...
                  'File_name',                          { 'DefaultField'   ,    @FrmFileName     } );
...
set(handles.ScfTypePopUp,         'Value',    sigClean.scfFormat                );
...
if isequal(sigClean.scfName, 0)
    ...
    set( [ handles.NumChanEditBox,  ...
           handles.NumSmplEditBox,  ...
           handles.PathEditBox ],   ...
           ...
           { 'String' } , { '' ; '' ; '>>> Path To .scf Data File' } );
    ...
    set( [ handles.NumChanEditBox,  ...
           ...
           ...
           { 'String' } ,           ...
           { int2str(sigClean.numChan) ; int2str(sigClean.numSmpl) ; fullfile( sigClean.scfPath , sigClean.scfName ) } );
...
function ScfPath(propName, propValue)

global sigClean;

[path name ext] = fileparts(propValue);

sigClean.scfName = [name ext];

sigClean.scfPath = fileparts(which(sigClean.scfName));

if isempty(sigClean.scfPath)
...
function ScfType(propName, propValue)
...
switch lower(propValue)
    ...
    case 'l'
        ...
        sigClean.scfFormat = 3;
        
    case 'p'
        
        sigClean.scfFormat = 4;
        
end     



function FrmFileName(propName, propValue)

global sigClean;

[path name ext] = fileparts(propValue);

sigClean.frmName = [name ext];

sigClean.frmPath = path;
